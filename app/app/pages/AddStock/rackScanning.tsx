...
export default function RackScanning() {
  ...
  useFocusEffect(
    React.useCallback(() => {
      console.log('RackScanning focused, resetting camera and scanned state');
  ...
  const submitToBackend = async (data: any) => {
    try {
      ...
      if (res.ok) {
        ...
        setResultMessage(result.message || 'Failed to add product.');
      }
    } catch (e) {
      setResultSuccess(false);
  ...
  const handleDoneOrRetry = () => {
    ...
    setScanned(false); // Allow scanning again if needed
    router.replace('/pages/AddStock/productScanning');
  };

  // Handle QR scan
  const handleBarcodeScanned = ({ data }: { data: string }) => {
    console.log('Barcode scanned event fired', { scanned, showRackModal, showResultModal });
    ...
    if (parsed.rackId) {
      ...
      }, 1200); // Show rack modal for 1.2s before result
    } else {
